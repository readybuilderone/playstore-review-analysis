...
st.header("HYPERGRYPH 话题评论分析POC")
_init_session_state()
...
if len(uploaded_file_list)>0:
    ...
    st.divider()
    st.info('概要信息', icon="ℹ️")
    num_unique_posts = st.session_state.reviewdata['post_id'].nunique()
    st.write(f"共有 {num_unique_posts} post")
    comment_counts = df.groupby('post_id')['comment_id'].count().sort_values(ascending=False).reset_index()
    st.write(comment_counts)
    ...
    if st.button("点击这个按钮，使用LLM分析评论", type="primary", use_container_width=True):
        with st.status("分析评论...", expanded=True):
            ...
            st.session_state.analyze_result = review_analyzer.analyze_hg_data(st.session_state.reviewdata.head(100), bedrock_chat)
